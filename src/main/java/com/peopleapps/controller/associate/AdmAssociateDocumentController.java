...
@Path("/associate/v1")
@Tag(name = "document by associate")
@RequestScoped
public class AdmAssociateDocumentController {
    ...
    @GET
    @Path("{key:[A-Za-z0-9-][A-Za-z0-9-]*}/document")
    ...
    public Response getALl(@PathParam("key") UUID key,
                           @QueryParam("descending") @DefaultValue("true") Boolean desc) {

        try {

            AdmPerson associate = admPersonRepository.findByKey(key);

            if (associate != null) {

                List<AdmAssociateDocumentDto> documentDtos = admDocumentRepository.getAllDocuments(
                        CsnConstants.ZERO,
                        associate.getDocumentAccount().getDocumentAccountId(), desc
                ...
                for (AdmAssociateDocumentDto dto : documentDtos) {
                    ...
                    strB.append("acodihue-core/rest/associate/v1/").append(key).append("/document/");
    ...
    @GET
    @Path("{key:[A-Za-z0-9-][A-Za-z0-9-]*}/document/{id:[0-9][0-9]*}")
    @RolesAllowed({RolesEnum.Constants.IT_VALUE, RolesEnum.Constants.OPERARIO_VALUE, RolesEnum.Constants.ADMINISTRADOR_VALUE})
    @Produces(MediaType.APPLICATION_JSON)
    public Response getALl(@PathParam("key") UUID key, @PathParam("id") Long id) {
    ...
    public Response update(@PathParam("key") UUID key,
        ...
        if (associate != null) {
            admDocumentRepository.saveEdit(new AdmDocument(
                            ...
                            new AdmDocumentAccount(
                                    associate.getDocumentAccount().getDocumentAccountId()
                            ),
                            null,
                            null,
                            null,
    ...
    @GET
    @Path("{key:[A-Za-z0-9-][A-Za-z0-9-]*}/document/{id:[0-9][0-9]*}/pdf")
